...
async function run() {
  try {
    ...
    await ssh.putFile('./src/app/globals.css', '/tmp/globals.css');
    await ssh.execCommand(`echo "Snyasi@098765" | sudo -S cp /tmp/globals.css /opt/smartvault/src/app/globals.css`);

    console.log('Uploading FileGrid.tsx...');
    await ssh.putFile('./src/components/FileGrid.tsx', '/tmp/FileGrid.tsx');
    await ssh.execCommand(`echo "Snyasi@098765" | sudo -S cp /tmp/FileGrid.tsx /opt/smartvault/src/components/FileGrid.tsx`);

    console.log('Uploading MainDashboard.tsx...');
